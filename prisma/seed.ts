...
import { LAGOS_LGAS } from '../src/config/lagos'
...
async function main() {
  ...
  // Official Lagos State publications distinguish these 20 LGAs from the separate LCDAs.
  // Coordinates are broad LGA-centre hints for map navigation, never residential positions.
  for (const lga of LAGOS_LGAS) {
    if (lga.slug === 'ojo-lga') continue // preserve the richer Ojo pilot record below
    const location = await prisma.location.upsert({ where: { slug: lga.slug }, update: { name: `${lga.name} LGA`, parentId: lagos.id, type: 'LGA', latitude: lga.center[1], longitude: lga.center[0], source: 'lagos_lga_seed', sourceReference: 'Lagos State Government: 20 Local Governments', verificationStatus: 'VERIFIED' }, create: { slug: lga.slug, name: `${lga.name} LGA`, parentId: lagos.id, type: 'LGA', latitude: lga.center[1], longitude: lga.center[0], source: 'lagos_lga_seed', sourceReference: 'Lagos State Government: 20 Local Governments', verificationStatus: 'VERIFIED' } })
    await prisma.community.upsert({ where: { slug: lga.slug }, update: { name: `${lga.name} LGA`, locationId: location.id, description: `Public ${lga.name} LGA community page. Local activity appears as it is added.` }, create: { slug: lga.slug, name: `${lga.name} LGA`, locationId: location.id, description: `Public ${lga.name} LGA community page. Local activity appears as it is added.` } })
  }
  const ojoLga = await prisma.location.upsert({ where: { slug: 'ojo-lga' }, update: { name: 'Ojo LGA', parentId: lagos.id, type: 'LGA', latitude: 6.462, longitude: 3.178, source: 'ojo_pilot_seed', sourceReference: 'Pilot locality list — requires local verification', verificationStatus: 'NEEDS_REVIEW' }, create: { slug: 'ojo-lga', name: 'Ojo LGA', parentId: lagos.id, type: 'LGA', latitude: 6.462, longitude: 3.178, source: 'ojo_pilot_seed', sourceReference: 'Pilot locality list — requires local verification', verificationStatus: 'NEEDS_REVIEW' } })
  await prisma.community.upsert({ where: { slug: 'ojo-lga' }, update: { name: 'Ojo LGA', locationId: ojoLga.id, description: 'Public Ojo LGA community page. Ojo pilot areas have deeper launch coverage.' }, create: { slug: 'ojo-lga', name: 'Ojo LGA', locationId: ojoLga.id, description: 'Public Ojo LGA community page. Ojo pilot areas have deeper launch coverage.' } })
